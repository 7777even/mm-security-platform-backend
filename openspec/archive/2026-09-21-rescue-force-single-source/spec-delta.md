# Spec Delta：rescue-resource（新增 capability）
...
### Requirement: 队伍体系子表作为救援力量真源（已废弃）

原「消防救援力量真源统一为队伍体系 `fac_brigade_*`」的口径移除；改由「救援力量唯一真源」需求承接
（`fac_brigade_team` 保留为中队主表，其子表退役）。
