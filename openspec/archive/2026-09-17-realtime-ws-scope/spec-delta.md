# Spec Delta: realtime-broadcast（实时广播 WS 握手鉴权 + 防区过滤）
...
## 关联 Spec
- 目标 spec 文件：`openspec/specs/realtime-broadcast/spec.md`（修改既有 capability，新增「WS 握手鉴权」「按 zone_codes 防区过滤」「防区注入扩展点 ZoneAware」三项需求）。
