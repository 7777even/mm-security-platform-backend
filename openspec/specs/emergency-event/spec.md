...
大屏应急指挥「应急事件」域数据源（`fac_emergency_event`，配 `fac_accident_incident`）：事件分组列表（只读）与登录态新增。写端点部分由 Change `2026-09-20-emergency-event-create` 回填；列表端点沿用既有 `GET /emergency-events`。
