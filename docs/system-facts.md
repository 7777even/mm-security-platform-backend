...
- **写端点授权覆盖度（2026-09-16 实测 + 已补门禁；2026-09-20 补应急事件写入）**：`*Controller.java` 下 **写端点 44 个**，其中声明 `perm=` 约束 **37 处**、`role=` **20 处**；代码引用的 23 个具体 perm 码**全部存在于 `sys_menu.perm_code` 种子**（交叉核对 0 缺失，故无「perm 拼错 → 全员 403」的隐患）。未带 `role=`/`perm=` 的仅 **8 个**，全部是**设计内**自助/入站端点，已逐条登记在 `scripts/check-endpoint-authz.mjs` 的 `ALLOWLIST` 并写明理由：`AuthController` 的 5 个（`/auth/{login,refresh,logout}` 属 `JwtFilter` 白名单；`/auth/password`·`/auth/profile` 为本人操作，靠 `AuthorizationService.assertSelfOrAdmin` 兜底）+ `UplinkController` 的 2 个（防爆手机现场回传、前端审计上报）+ `EmergencyEventController` 的 1 个（大屏新增事件/演练/极端天气，登录态自助、按变更决策取非 ADMIN）。
