...
- 达梦 DM8 / PG 暂缓（本机无实例；PG 已于 2026-09-17 用 Docker 实跑通过）；`application-dm.yml` 与 `db/migration/dameng` 已补齐至 **V57** 并完成三方言静态一致性校验（版本/表/列名三方言对齐），**达梦 `;` 终结符于 2026-09-17 才真正补齐**（09-16 一轮实测不完整，见下），仅待 DM8 实例 `flyway migrate` 实跑复核。代码层 DB 无关（MyBatis-Plus 方言探测、不写方言函数）。
