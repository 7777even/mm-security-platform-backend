...
- 2026-09-16（AI 过程治理补齐）：① 本库 `AGENTS.md` 新增 **§0「跨库入口」**（复述仓库根伞文件，因根 `mm-security-platform/` 不是 git 仓库、伞文件不入库，clone 单库曾读不到跨库约束）；② `.cursor/rules/` 新增 `openapi-contract-writing.mdc`（契约写法由 `templates/` 迁一份为 harness 自动加载项）；③ 新增 `engineering/reports/`（干系人阶段简报层，与 `qa/`+`retro/` 分工），`templates/_stage_report_template.md` 落点由 `plans/` 修正为 `reports/`；④ 新增 `docs/architecture/alarm-three-surfaces.md`（报警三端口径设计真源，固化 `fac_alarm`/`fac_fire_alarm`/`fac_production_alarm` 为何并存）；⑤ 根目录 `openspec/changes/2026-09-14-mgmt-alarm-record/` 残留副本已清除（正式版在前端库 `openspec/archive/`）。**DB-IT 口径澄清**：本库 `*IT.java` 实测 **1 个**（`integration/DbLayerIntegrationIT`，H2 复用 Flyway V），`testcontainers` 引用 **0**——对标分析中的 P1-1 已据此由 ✅ 改判 🟡（详见 `docs/AI编程规范与过程资产对标分析.md` §4）。
