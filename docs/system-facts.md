...
- 达梦 DM8 / PG 暂缓（本机无实例、无 docker）；`application-dm.yml` 与 `db/migration/dameng` 已补齐至 **V57** 并完成三方言静态一致性校验（版本/表/列名三方言对齐、`;` 终结符齐），仅待 DM8 实例 `flyway migrate` 实跑复核。代码层 DB 无关（MyBatis-Plus 方言探测、不写方言函数）。
...
- 达梦迁移脚本收口（2026-09-16）：V51–V57 三方言补齐 + 24 个回填文件补 `;` 终结符（对齐 V1/V2 与 h2/pg 惯例） + 新增 `scripts/check-dialect-migration-consistency.py`（三方言静态一致性校验，已接入后端 CI `dialect-consistency` job）与 `scripts/fix-dameng-terminators.py`。校验结果 EXIT=0：版本 57/57、表 140、列名三方言对齐，仅余 7 条良性 `[TYPE]` 告警（布尔标志列 h2=TINYINT / pg=SMALLINT / dameng=NUMBER(1)，均存 0/1，运行无害）。**达梦/PG 仍未在真实实例实跑**。
