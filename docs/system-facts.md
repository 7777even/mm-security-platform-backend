...
- 达梦 DM8 / PG 暂缓（本机无实例、无 docker）；`application-dm.yml` 与 `db/migration/dameng` 保留作迁移资产。代码层 DB 无关（MyBatis-Plus 方言探测、不写方言函数）。
  > **版本缺口（2026-09-11 实测，持续更新）**：h2 已至 **V50**；dameng/postgresql 截至 2026-09-14 已补齐 **V46–V50** 镜像（达梦 `NUMBER(19) GENERATED BY DEFAULT AS IDENTITY` / `VARCHAR2` / 逐条 INSERT、PG `BIGSERIAL` / `VARCHAR` / 多行 VALUES；文件头均标注**未实跑验证**，待 DM8/PG 环境激活后 flyway 校验）。两库仍缺 **V4–V25、V27–V31、V44–V45**（V44 为 h2 单方言引用 `fac_production_personnel`/`fac_workstation` 且含 `BOOLEAN TRUE/FALSE`，达梦不认，镜像暂缓）。**结论：dameng/pg 仍不可直接上生产（缺 V4–V25 等大批表会让 Flyway 报红）；V46–V50 补齐仅消除该区间缺口，非全量对齐**；「暂缓」指的就是这个状态，不是「已对齐」。
