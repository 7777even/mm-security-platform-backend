...
- **默认管理员一次性种子**：`AuthService.ensureAdmin()` 仅在 `sys_user` 表为空时写入 `admin / admin@2026`；非空即跳过，不在运行时反复重置。
...
| 默认账号种子 | `service/AuthService.ensureAdmin()` | ✅ |
...
- `password_hash` 字段**永不出响应体**（DTO 映射排除；若需返回用户信息走 `AuthService.me()` 的 `Map`，只含 `username/realName/role`）。
