...
- **RBAC 模型**：角色存 `sys_user.role`（`ADMIN` / 普通角色），菜单可见性由 `sys_menu.allowed_roles`（逗号分隔）按当前角色过滤（`AuthService.menus()`）。端点级角色门禁用 `@RequireAuth(role=...)`；资源级归属校验用 `AuthorizationService.assertSelfOrAdmin`。
...
| 默认管理员 | `AuthService.ensureAdmin()`（`admin` / `admin@2026`，表空时种子） | ✅ 已落地 |
