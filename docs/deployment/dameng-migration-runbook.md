...
### 5.1 实际采用的方式：绕过 Flyway，用 disql 逐文件执行

⚠️ **Flyway 社区版与商业版均不内置达梦 database 模块** ⇒ 本机验证**不走 Flyway**，改用达梦自带 `disql` 逐文件执行 `db/migration/dameng/` 下的 V1–V57，以验证「SQL 本身能否被 DM8 接受」。执行器：`scripts/verify-dameng-migration.cmd`。
...
### 5.2 实测结论（2026-09-17，本机 DM8 开发版）
...
- **仍未验证**：Flyway 集成本身（需第三方达梦 database 模块或自实现 DatabaseType SPI），以及应用以 `--spring.profiles.active=dm` 启动后的端到端冒烟（见 §6）。

### 5.3 静态复核（每次改迁移后，CI 已自动跑）
