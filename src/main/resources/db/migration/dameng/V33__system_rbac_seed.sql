-- =============================================================================
-- V33 系统管理域 RBAC 种子 —— 达梦 DM8 方言（Oracle 兼容）
--   与 H2 V33 对齐。DM8 无官方 Flyway 模块，本文件为迁移资产，须到达梦实例上复核执行。
--   注意：Oracle 兼容语法不支持多行 VALUES，故角色种子拆为逐条 INSERT；
--        纯常量 SELECT 需 FROM dual。
-- =============================================================================

INSERT INTO sys_role (role_code, role_name, description, data_scope, status, built_in, sort_order) VALUES ('ADMIN',       '系统管理员', '内置超级管理员，拥有全部菜单与权限', 'ALL',  1, 1, 0);
INSERT INTO sys_role (role_code, role_name, description, data_scope, status, built_in, sort_order) VALUES ('COMMANDER',   '总指挥',     '园区应急总指挥',                     'ALL',  1, 0, 10);
INSERT INTO sys_role (role_code, role_name, description, data_scope, status, built_in, sort_order) VALUES ('SCHEDULER',   '值班调度',   '值班调度岗',                         'DEPT', 1, 0, 20);
INSERT INTO sys_role (role_code, role_name, description, data_scope, status, built_in, sort_order) VALUES ('TEAM_LEADER', '属地班长',   '属地运行部班长',                     'DEPT', 1, 0, 30);
INSERT INTO sys_role (role_code, role_name, description, data_scope, status, built_in, sort_order) VALUES ('INNER_OPER',  '内操',       '装置内操',                           'SELF', 1, 0, 40);
INSERT INTO sys_role (role_code, role_name, description, data_scope, status, built_in, sort_order) VALUES ('OUTER_OPER',  '外操',       '装置外操',                           'SELF', 1, 0, 50);
