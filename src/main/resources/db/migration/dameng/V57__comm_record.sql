-- V57 通讯通知记录域（后台管理端「通讯通知管理」五个记录页）真后端化（达梦方言）
...
-- 列名用 result_text 而非 result：回避达梦保留字冲突（Java 字段仍为 result，@TableField 映射）。

CREATE TABLE fac_comm_record (
    id NUMBER(19) IDENTITY(1,1) PRIMARY KEY,
    record_no VARCHAR2(64 CHAR) NOT NULL,
    record_type VARCHAR2(16 CHAR) NOT NULL,
    occurred_at VARCHAR2(32 CHAR),
    category VARCHAR2(64 CHAR),
    sender VARCHAR2(64 CHAR),
    receiver VARCHAR2(128 CHAR),
    summary VARCHAR2(512 CHAR),
    result_text VARCHAR2(32 CHAR),
    duration VARCHAR2(16 CHAR),
    channel VARCHAR2(64 CHAR),
    direction VARCHAR2(16 CHAR),
    content_type VARCHAR2(16 CHAR)
);
SET IDENTITY_INSERT fac_comm_record ON;
...
SET IDENTITY_INSERT fac_comm_record OFF;
