-- =============================================================================
-- V28 安防轨迹扩展列 —— 达梦 DM8 方言（Oracle 兼容，未实跑验证）
--   镜像自 h2/V28__security_track.sql。本文件未经达梦实例实跑验证，按 DM8 语法编写，需上环境复核。
-- =============================================================================
...
ALTER TABLE fac_vehicle_search ADD vehicle_type VARCHAR2(32 CHAR);

ALTER TABLE fac_vehicle_search ADD driver_name VARCHAR2(32 CHAR);

ALTER TABLE fac_vehicle_search ADD driver_phone VARCHAR2(32 CHAR);

ALTER TABLE fac_vehicle_search ADD company VARCHAR2(64 CHAR);

ALTER TABLE fac_vehicle_search ADD appointment_no VARCHAR2(32 CHAR);

ALTER TABLE fac_vehicle_search ADD appointment_time VARCHAR2(64 CHAR);

ALTER TABLE fac_vehicle_search ADD visit_purpose VARCHAR2(64 CHAR);

ALTER TABLE fac_vehicle_search ADD waybill_no VARCHAR2(32 CHAR);

ALTER TABLE fac_vehicle_search ADD cargo VARCHAR2(64 CHAR);

ALTER TABLE fac_vehicle_search ADD destination VARCHAR2(64 CHAR);


-- 2) 人员检索详情扩展列（原 securitySearchMock.personDetailExtras 迁入 DB）
ALTER TABLE fac_person_search ADD gender VARCHAR2(8 CHAR);

ALTER TABLE fac_person_search ADD phone VARCHAR2(32 CHAR);

ALTER TABLE fac_person_search ADD company VARCHAR2(64 CHAR);

ALTER TABLE fac_person_search ADD id_number VARCHAR2(32 CHAR);

ALTER TABLE fac_person_search ADD appointment_no VARCHAR2(32 CHAR);

ALTER TABLE fac_person_search ADD appointment_time VARCHAR2(64 CHAR);

ALTER TABLE fac_person_search ADD visit_purpose VARCHAR2(64 CHAR);

ALTER TABLE fac_person_search ADD special_operation VARCHAR2(64 CHAR);

ALTER TABLE fac_person_search ADD operation_area VARCHAR2(64 CHAR);


-- 3) 轨迹时间轴表（entity_id 为 NULL 的行表示该模式下的默认轨迹）
CREATE TABLE fac_security_track (
    id NUMBER(19) IDENTITY(1,1) PRIMARY KEY,
    track_mode VARCHAR2(16 CHAR) NOT NULL,
    entity_id NUMBER(19) NULL,
    seq_no NUMBER(9) NOT NULL,
    location VARCHAR2(64 CHAR),
    status VARCHAR2(32 CHAR),
    status_tone VARCHAR2(16 CHAR),
    track_time VARCHAR2(32 CHAR),
    capture_hint VARCHAR2(64 CHAR)
);


-- 4) 轨迹模式配置表（起止点标签）
CREATE TABLE fac_security_track_meta (
    track_mode VARCHAR2(16 CHAR) PRIMARY KEY,
    start_label VARCHAR2(32 CHAR),
    end_label VARCHAR2(32 CHAR)
);
