-- V51 管理台账通用能力（达梦 DM8 方言 Oracle 兼容，未实跑验证）
CREATE TABLE mgmt_ledger_meta (
    id NUMBER(19) IDENTITY(1,1) PRIMARY KEY,
    domain_code VARCHAR2(64 CHAR) NOT NULL,
    title VARCHAR2(128 CHAR) NOT NULL,
    columns_json VARCHAR2(4000 CHAR),
    filter_json VARCHAR2(2000 CHAR),
    sort_no NUMBER(9) NOT NULL DEFAULT 0
);
CREATE TABLE mgmt_ledger_row (
    id NUMBER(19) IDENTITY(1,1) PRIMARY KEY,
    domain_code VARCHAR2(64 CHAR) NOT NULL,
    row_no NUMBER(9) NOT NULL DEFAULT 0,
    sort_no NUMBER(9) NOT NULL DEFAULT 0
);
CREATE TABLE mgmt_ledger_cell (
    id NUMBER(19) IDENTITY(1,1) PRIMARY KEY,
    row_id NUMBER(19) NOT NULL,
    col_index NUMBER(9) NOT NULL DEFAULT 0,
    col_key VARCHAR2(64 CHAR),
    cell_text VARCHAR2(1024 CHAR),
    cell_type VARCHAR2(8 CHAR)
);
