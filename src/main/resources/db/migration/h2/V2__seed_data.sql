-- =============================================================================
-- V2 种子数据（dev 联调用，生产亦可复用）—— H2 方言
--   默认 admin 账号由 AuthService.ensureAdmin() 写入（密码 admin@2026，BCrypt 同步校验），
--   不在此处落种子，避免与业务初始化逻辑重复。
--   源真值：原 src/main/resources/data.sql。
-- =============================================================================
...
-- 设备种子数据：device_code 为 20 位 MDM 物理主键（与 byCode/page 的 20 位校验一致）。
...
-- 告警种子数据：status 用后端 int（0=ACTIVE/1=ACKED/2=DISPATCHED/3=CLOSED），由 AlarmAssembler 映射为 string 枚举；
