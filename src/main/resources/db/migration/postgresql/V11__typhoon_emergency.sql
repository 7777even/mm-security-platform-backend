-- V11 台风应急大屏真实数据源：台风应急事件聚合 + 防汛排涝力量清单。 —— PostgreSQL 方言（未实跑验证）
-- 镜像自 h2/V11。本文件未经 PostgreSQL 实例实跑验证，按标准 PG 语法编写，需上环境复核。
...
CREATE TABLE IF NOT EXISTS fac_typhoon_incident (
...
CREATE TABLE IF NOT EXISTS fac_typhoon_monitor_object (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_typhoon_risk_warning (
    id BIGSERIAL PRIMARY KEY,
    incident_id BIGINT NOT NULL,
...
CREATE TABLE IF NOT EXISTS fac_typhoon_live_video (
    id BIGSERIAL PRIMARY KEY,
    incident_id BIGINT NOT NULL,
...
CREATE TABLE IF NOT EXISTS fac_typhoon_map_risk_point (
    id BIGSERIAL PRIMARY KEY,
    incident_id BIGINT NOT NULL,
...
CREATE TABLE IF NOT EXISTS fac_typhoon_series (
    id BIGSERIAL PRIMARY KEY,
    incident_id BIGINT NOT NULL,
...
CREATE TABLE IF NOT EXISTS fac_typhoon_event_info (
    id BIGSERIAL PRIMARY KEY,
    incident_id BIGINT NOT NULL,
...
CREATE TABLE IF NOT EXISTS fac_typhoon_aux_item (
    id BIGSERIAL PRIMARY KEY,
    incident_id BIGINT NOT NULL,
...
CREATE TABLE IF NOT EXISTS fac_typhoon_dispatch_resource (
    id BIGSERIAL PRIMARY KEY,
