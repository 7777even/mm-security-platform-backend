-- V14 视频控制/视频墙大屏真实数据源（fm-video-control / fm-video-wall）。 —— PostgreSQL 方言（未实跑验证）
-- 镜像自 h2/V14。本文件未经 PostgreSQL 实例实跑验证，按标准 PG 语法编写，需上环境复核。
...
CREATE TABLE IF NOT EXISTS fac_video_group (
...
CREATE TABLE IF NOT EXISTS fac_video_camera (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_video_linkage (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_video_linkage_rule (
    id BIGSERIAL PRIMARY KEY,
    config_code VARCHAR(32) NOT NULL,
