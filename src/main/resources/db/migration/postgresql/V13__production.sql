-- V13 生产应急监测大屏真实数据源（fm-production / fm-production-area）。 —— PostgreSQL 方言（未实跑验证）
-- 镜像自 h2/V13。本文件未经 PostgreSQL 实例实跑验证，按标准 PG 语法编写，需上环境复核。
...
CREATE TABLE IF NOT EXISTS fac_production_facility (
...
CREATE TABLE IF NOT EXISTS fac_production_device_category (
    id BIGSERIAL PRIMARY KEY,
    name VARCHAR(64) NOT NULL,
    item_count INT NOT NULL DEFAULT 0,
    image VARCHAR(128),
...
CREATE TABLE IF NOT EXISTS fac_production_stat (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_production_alarm (
    id BIGSERIAL PRIMARY KEY,
    facility_id BIGINT,
...
CREATE TABLE IF NOT EXISTS fac_production_risk_warning (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_production_personnel (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_production_device (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_production_area_metric (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_production_area_zone (
    id BIGSERIAL PRIMARY KEY,
    facility_id BIGINT NOT NULL,
