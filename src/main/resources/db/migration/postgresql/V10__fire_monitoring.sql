-- V10 消防监控大屏真实数据源：消防救援力量统计 / 特殊作业统计 / 消防设施设备状态 / 防火巡查记录。 —— PostgreSQL 方言（未实跑验证）
-- 镜像自 h2/V10。本文件未经 PostgreSQL 实例实跑验证，按标准 PG 语法编写，需上环境复核。
...
CREATE TABLE IF NOT EXISTS fac_rescue_force_stat (
...
CREATE TABLE IF NOT EXISTS fac_special_operation_stat (
    id BIGSERIAL PRIMARY KEY,
    label VARCHAR(32) NOT NULL,
    stat_count INT NOT NULL DEFAULT 0,
    sort_no INT NOT NULL DEFAULT 0
...
CREATE TABLE IF NOT EXISTS fac_fire_equipment_status (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_fire_patrol (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_fire_patrol_item_def (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_fire_patrol_item_result (
    id BIGSERIAL PRIMARY KEY,
