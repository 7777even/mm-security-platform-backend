-- V22 通讯设备 + 天气观测/预报真实数据源。 —— PostgreSQL 方言（未实跑验证）
-- 镜像自 h2/V22。本文件未经 PostgreSQL 实例实跑验证，按标准 PG 语法编写，需上环境复核。
...
-- ---------------------------------------------------------------- 通讯设备
CREATE TABLE IF NOT EXISTS fac_comm_device (
...
-- ---------------------------------------------------------------- 天气实况（单行）
CREATE TABLE IF NOT EXISTS fac_weather_current (
    id BIGSERIAL PRIMARY KEY,
...
-- ---------------------------------------------------------------- 逐小时序列（8 点）
CREATE TABLE IF NOT EXISTS fac_weather_hourly (
    id BIGSERIAL PRIMARY KEY,
...
-- ---------------------------------------------------------------- 七日预报（7 天）
CREATE TABLE IF NOT EXISTS fac_weather_daily (
    id BIGSERIAL PRIMARY KEY,
