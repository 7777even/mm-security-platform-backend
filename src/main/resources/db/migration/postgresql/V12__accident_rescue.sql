-- V12 事故救援应急大屏真实数据源：事件聚合 + 详情字段 + 调度资源 + 值班人员 + 辅助统计 + 动态快讯。 —— PostgreSQL 方言（未实跑验证）
-- 镜像自 h2/V12。本文件未经 PostgreSQL 实例实跑验证，按标准 PG 语法编写，需上环境复核。
...
CREATE TABLE IF NOT EXISTS fac_accident_incident (
...
CREATE TABLE IF NOT EXISTS fac_accident_detail_field (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_accident_dispatch_resource (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_accident_duty_person (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_accident_aux_stat (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_accident_dynamic (
    id BIGSERIAL PRIMARY KEY,
