-- V24 大屏面板遗留硬编码数据落地（消防设备分类 / 系统消息条 / 工业电视地图撒点 / 视频监控档案）。 —— PostgreSQL 方言（未实跑验证）
-- 镜像自 h2/V24。本文件未经 PostgreSQL 实例实跑验证，按标准 PG 语法编写，需上环境复核。
...
CREATE TABLE IF NOT EXISTS fac_fire_equipment_category (
...
CREATE TABLE IF NOT EXISTS fac_system_message (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_tv_map_point (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_tv_monitor (
    id BIGSERIAL PRIMARY KEY,
