-- V4 安全防恐域：巡逻摄像机 / 道闸 / 防恐柱 / 车辆·人员识别检索 / 门禁事件 —— PostgreSQL 方言（未实跑验证）
-- 镜像自 h2/V4。本文件未经 PostgreSQL 实例实跑验证，按标准 PG 语法编写，需上环境复核。
...
CREATE TABLE IF NOT EXISTS fac_patrol_camera (
...
CREATE TABLE IF NOT EXISTS fac_gate_control (
    id       BIGINT PRIMARY KEY,
    name     VARCHAR(128),
    location VARCHAR(64),
    status   VARCHAR(16),
...
CREATE TABLE IF NOT EXISTS fac_bollard (
    id       BIGINT PRIMARY KEY,
    name     VARCHAR(128),
    zone     VARCHAR(32),
    status   VARCHAR(16),
...
CREATE TABLE IF NOT EXISTS fac_vehicle_search (
    id         BIGINT PRIMARY KEY,
    plate      VARCHAR(32),
    confidence INT,
    gate       VARCHAR(32),
...
CREATE TABLE IF NOT EXISTS fac_person_search (
    id     BIGINT PRIMARY KEY,
    name   VARCHAR(32),
    gate   VARCHAR(32),
    status VARCHAR(16),
...
CREATE TABLE IF NOT EXISTS fac_security_event (
