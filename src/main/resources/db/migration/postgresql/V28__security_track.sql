-- V28 巡更/通行轨迹 + 检索详情扩展（替代前端 securityTrackMock / securitySearchMock 的假数据）。 —— PostgreSQL 方言（未实跑验证）
-- 镜像自 h2/V28。本文件未经 PostgreSQL 实例实跑验证，按标准 PG 语法编写，需上环境复核。
...
CREATE TABLE IF NOT EXISTS fac_security_track (
...
-- 4) 轨迹模式配置表（起止点标签）
CREATE TABLE IF NOT EXISTS fac_security_track_meta (
