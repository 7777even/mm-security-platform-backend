-- V18 应急预案大屏真实数据源（预案切换面板 + 预案矩阵）。 —— PostgreSQL 方言（未实跑验证）
-- 镜像自 h2/V18。本文件未经 PostgreSQL 实例实跑验证，按标准 PG 语法编写，需上环境复核。
...
CREATE TABLE IF NOT EXISTS fac_emergency_plan (
...
CREATE TABLE IF NOT EXISTS fac_plan_instance (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_plan_major_phase (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_plan_sub_phase (
    id BIGSERIAL PRIMARY KEY,
    instance_id BIGINT NOT NULL,
    phase_code VARCHAR(32) NOT NULL,
...
CREATE TABLE IF NOT EXISTS fac_plan_risk_event (
    id BIGSERIAL PRIMARY KEY,
    instance_id BIGINT NOT NULL,
...
CREATE TABLE IF NOT EXISTS fac_plan_resource (
    id BIGSERIAL PRIMARY KEY,
    instance_id BIGINT NOT NULL,
...
CREATE TABLE IF NOT EXISTS fac_plan_action_card (
    id BIGSERIAL PRIMARY KEY,
    instance_id BIGINT NOT NULL,
