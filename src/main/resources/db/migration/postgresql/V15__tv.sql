-- V15 工业电视大屏真实数据源（fm-tv）。 —— PostgreSQL 方言（未实跑验证）
-- 镜像自 h2/V15。本文件未经 PostgreSQL 实例实跑验证，按标准 PG 语法编写，需上环境复核。
...
CREATE TABLE IF NOT EXISTS fac_tv_stat_item (
...
CREATE TABLE IF NOT EXISTS fac_tv_operation_stat (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_tv_inspection_record (
    id BIGSERIAL PRIMARY KEY,
