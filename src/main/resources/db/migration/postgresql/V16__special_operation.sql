-- V16 特殊作业大屏真实数据源（壳内特殊作业面板 + 消防特殊作业弹窗复用）。 —— PostgreSQL 方言（未实跑验证）
-- 镜像自 h2/V16。本文件未经 PostgreSQL 实例实跑验证，按标准 PG 语法编写，需上环境复核。
...
CREATE TABLE IF NOT EXISTS fac_special_operation_ticket (
...
CREATE TABLE IF NOT EXISTS fac_special_operation_video (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fac_special_operation_gas (
    id BIGSERIAL PRIMARY KEY,
    ticket_id BIGINT NOT NULL,
    name VARCHAR(64) NOT NULL,
...
CREATE TABLE IF NOT EXISTS fac_special_operation_person (
    id BIGSERIAL PRIMARY KEY,
    ticket_id BIGINT NOT NULL,
